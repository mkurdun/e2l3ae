...
class InteractionBatchDataset(keras.utils.PyDataset):
    ...
    def __init__(
        ...
        max_queue_size=10,
        max_output=None,
        include_full_user_row: bool = False,
    ):
        ...
        self.device = device
        self.include_full_user_row = bool(include_full_user_row)
        self.max_output = X.shape[1] if max_output is None else max_output
    ...
    def __getitem__(self, n):
        ...
        user_row_idx = self.indices[ind_min:ind_max]
        M = self.X[user_row_idx]
        ...
        xy = (torch_coo_x.to(self.device).to_dense(), torch_coo_y.to(self.device).to_dense())
        if self.include_full_user_row:
            X_full = torch.from_numpy(self.X[user_row_idx].toarray().astype(np.float32)).to(self.device)
            xy = (*xy, X_full)
        ...
        return xy, (
            ...
            torch.from_numpy(item_slicer_with_negatives.astype(np.int64)).long(),
            torch.from_numpy(user_row_idx.astype(np.int64)).long().to(self.device),
        )
